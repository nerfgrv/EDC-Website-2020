...
def PostCreateView(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Your submission has been sent to our team for review. You will be notified via e-mail if it is published.')
...
def PostUpdateView(request, pk):
    if request.method == 'POST':
        form = PostUpdateForm(request.POST, request.FILES, instance=Post.objects.filter(id=pk).first())
        if form.is_valid():
            if request.user.is_team:
                ...
                messages.warning(request, 'You do not have the perimission to update this blog.')
...
def magazine(request):
    return render(request,'blog/bizfinatics.html')